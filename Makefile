...
install-js: ## Install JS package dependencies
	cd crates/ouros-js && npm install
...
dev-py: ## Install the python package for development
	uv run maturin develop --uv -m crates/ouros-python/Cargo.toml

.PHONY: dev-js
dev-js: ## Build the JS package (debug)
	cd crates/ouros-js && npm run build:debug

.PHONY: lint-js
lint-js: install-js ## Lint JS code with oxlint
	cd crates/ouros-js && npm run lint

.PHONY: test-js
test-js: dev-js ## Build and test the JS package
	cd crates/ouros-js && npm test

.PHONY: smoke-test-js
smoke-test-js: ## Run smoke test for JS package (builds, packs, and tests installation)
	cd crates/ouros-js && npm run smoke-test

.PHONY: dev-py-release
dev-py-release: ## Install the python package for development with a release build
	uv run maturin develop --uv -m crates/ouros-python/Cargo.toml --release

.PHONY: dev-js-release
dev-js-release: ## Build the JS package (release)
	cd crates/ouros-js && npm run build

.PHONY: dev-py-pgo
dev-py-pgo: ## Install the python package for development with profile-guided optimization
	$(eval PROFDATA := $(shell mktemp -d))
	RUSTFLAGS='-Cprofile-generate=$(PROFDATA)' uv run maturin develop --uv -m crates/ouros-python/Cargo.toml --release
	uv run --package ouros --only-dev pytest crates/ouros-python/tests -k "not test_parallel_exec"
	...
	RUSTFLAGS='-Cprofile-use=$(PROFDATA)/merged.profdata' $(uv-run-no-sync) maturin develop --uv -m crates/ouros-python/Cargo.toml --release
	@rm -rf $(PROFDATA)
...
format-js: install-js ## Format JS code with prettier
	cd crates/ouros-js && npm run format:prettier
...
lint-py: dev-py ## Lint Python code with ruff
	...
	uv run -m mypy.stubtest ouros._ouros --ignore-disjoint-bases
...
test-ref-count-panic: ## Run rust tests with ref-count-panic enabled
	cargo test -p ouros --features ref-count-panic

.PHONY: test-ref-count-return
test-ref-count-return: ## Run rust tests with ref-count-return enabled
	cargo test -p ouros --features ref-count-return

.PHONY: test-cases
test-cases: ## Run tests cases only
	cargo test -p ouros --test datatest_runner

.PHONY: test-type-checking
test-type-checking: ## Run rust tests on ouros_type_checking
	cargo test -p ouros_type_checking -p ouros_typeshed

.PHONY: pytest
pytest: ## Run Python tests with pytest
	uv run --package ouros --only-dev pytest crates/ouros-python/tests
...
test-docs: dev-py ## Test docs examples only
	uv run --package ouros --only-dev pytest crates/ouros-python/tests/test_readme_examples.py
	cargo test --doc -p ouros

.PHONY: test
test: test-ref-count-panic test-ref-count-return test-no-features test-type-checking test-py ## Run rust tests

.PHONY: repl
repl: ## Build the interactive REPL binary
	cargo build -p ouros --bin ouros-repl
...
testcov: ## Run Rust tests with coverage, print table, and generate HTML report
	...
	cargo llvm-cov --no-report -p ouros
	echo "coverage for `make test-ref-count-panic`"
	cargo llvm-cov --no-report -p ouros --features ref-count-panic
	echo "coverage for `make test-ref-count-return`"
	cargo llvm-cov --no-report -p ouros --features ref-count-return
	echo "coverage for `make test-type-checking`"
	cargo llvm-cov --no-report -p ouros_type_checking -p ouros_typeshed
...
update-typeshed: ## Update vendored typeshed from upstream
	uv run crates/ouros-typeshed/update.py
	uv run ruff format
	uv run ruff check --fix --fix-only --silent

.PHONY: bench
bench: ## Run benchmarks
	cargo bench -p ouros --bench main

.PHONY: dev-bench
dev-bench: ## Run benchmarks to test with dev profile
	cargo bench --profile dev -p ouros --bench main -- --test

.PHONY: profile
profile: ## Profile the code with pprof and generate flamegraphs
	cargo bench -p ouros --bench main --profile profiling -- --profile-time=10
...
type-sizes: ## Write type sizes for the crate to ./type-sizes.txt (requires nightly and top-type-sizes)
	RUSTFLAGS="-Zprint-type-sizes" cargo +nightly build -j1 2>&1 | top-type-sizes -f '^ouros.*' > type-sizes.txt
