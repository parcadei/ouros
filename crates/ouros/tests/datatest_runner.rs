...
use ahash::AHashMap;
use ouros::{
    ExcType, Exception, ExternalResult, LimitedTracker, Object, OsFunction, OurosFuture, ResourceLimits, RunProgress,
    Runner, StdPrint, dir_stat, file_stat,
};
...
///
/// Used for both Ouros and CPython tests. CPython needs ~5 extra frames
...
///
/// Parsed from an optional first-line comment like `# xfail=ouros,cpython` or `# call-external`.
/// If not present, defaults to running on both interpreters in standard mode.
///
/// ## Xfail Semantics (Strict)
/// - `xfail=ouros` - Test is expected to fail on Ouros; if it passes, that's an error
/// - `xfail=cpython` - Test is expected to fail on CPython; if it passes, that's an error
/// - `xfail=ouros,cpython` - Expected to fail on both interpreters
#[derive(Debug, Clone, Default)]
#[expect(clippy::struct_excessive_bools)]
struct TestConfig {
    /// When true, test is expected to fail on Ouros (strict xfail).
    xfail_ouros: bool,
    ...
    /// Used for tests with top-level await which Ouros supports but CPython doesn't.
    async_mode: bool,
}
...
enum Expectation {
    ...
    /// The expected traceback string should match exactly between Ouros and CPython.
    Traceback(String),
...
///
/// The file may optionally contain a `# xfail=ouros,cpython` comment to specify
...
const ITER_EXT_FUNCTIONS: &[&str] = &[
    ...
    "async_raise",        // (exc_type: str, message: str) -> async: raises exception
];
...
/// iter mode tests produce identical results in both Ouros and CPython.
///
...
enum DispatchResult {
    ...
    Sync(ExternalResult),
    /// Asynchronous call that resolves successfully.
    AsyncReturn(Object),
    /// Asynchronous call that resolves with an exception.
...
fn dispatch_external_call(name: &str, args: Vec<Object>) -> DispatchResult {
    match name {
        "add_ints" => {
            ...
            DispatchResult::Sync(Object::Int(a + b).into())
        }
        "concat_strings" => {
            ...
            DispatchResult::Sync(Object::String(a + &b).into())
        }
        "return_value" => {
        ...
        "get_list" => {
            ...
            DispatchResult::Sync(Object::List(vec![Object::Int(1), Object::Int(2), Object::Int(3)]).into())
        }
        "raise_error" => {
            // raise_error(exc_type: str, message: str) -> raises exception
            ...
            DispatchResult::Sync(Exception::new(exc_type, Some(message)).into())
        }
        "make_point" => {
            ...
            DispatchResult::Sync(
                Object::Dataclass {
                    ...
                    attrs: vec![
                        ...
                        (Object::String("y".to_string()), Object::Int(2)),
                    ]
                    .into(),
                    methods: vec![],
                    frozen: true,
        ...
        "make_user" => {
            ...
            DispatchResult::Sync(
                Object::Dataclass {
                    ...
                    attrs: vec![
                        (Object::String("name".to_string()), Object::String(name)),
                        (Object::String("active".to_string()), Object::Bool(true)),
                    ]
                    .into(),
                    methods: vec![],
                    frozen: true,
        ...
        "async_call" => {
            ...
            DispatchResult::AsyncReturn(args.into_iter().next().unwrap())
        }
        "async_raise" => {
            // async_raise(exc_type: str, message: str) -> async function that raises
            // the requested exception when resumed.
            assert!(args.len() == 2, "async_raise requires 2 arguments");
            let exc_type_str = String::try_from(&args[0]).expect("async_raise: first arg must be str");
            let message = String::try_from(&args[1]).expect("async_raise: second arg must be str");
            let exc_type = match exc_type_str.as_str() {
                "ValueError" => ExcType::ValueError,
                "TypeError" => ExcType::TypeError,
                "KeyError" => ExcType::KeyError,
                "RuntimeError" => ExcType::RuntimeError,
                _ => panic!("async_raise: unsupported exception type: {exc_type_str}"),
            };
            DispatchResult::AsyncError(Exception::new(exc_type, Some(message)))
        }
...
fn get_kwarg_bool(kwargs: &[(Object, Object)], name: &str) -> bool {
    for (key, value) in kwargs {
        if let Object::String(key_str) = key
            && key_str == name
        {
            return matches!(value, Object::Bool(true));
        }
    }
    false
}
...
///
/// Returns an `ExternalResult` to pass back to the Ouros interpreter.
...
fn dispatch_os_call(function: OsFunction, args: &[Object], kwargs: &[(Object, Object)]) -> ExternalResult {
    ...
    if function == OsFunction::GetEnviron {
        ...
        let env_dict = vec![
            (
                Object::String("VIRTUAL_HOME".to_owned()),
                Object::String("/virtual/home".to_owned()),
            ),
            (
                Object::String("VIRTUAL_USER".to_owned()),
                Object::String("testuser".to_owned()),
            ),
            (
                Object::String("VIRTUAL_EMPTY".to_owned()),
                Object::String(String::new()),
            ),
        ];
        return Object::Dict(env_dict.into()).into();
    }

    // Extract path from Object::Path (or String for backwards compatibility)
    let path = match &args[0] {
        Object::Path(p) => p.clone(),
    ...
    match function {
        ...
        OsFunction::ReadText => {
            if let Some(file) = get_virtual_file(&path) {
                match std::str::from_utf8(&file.content) {
                    Ok(text) => Object::String(text.to_owned()).into(),
                    Err(_) => Exception::new(
        ...
        OsFunction::ReadBytes => {
            if let Some(file) = get_virtual_file(&path) {
                Object::Bytes(file.content).into()
            } else {
                Exception::new(
                    ExcType::FileNotFoundError,
                    Some(format!("[Errno 2] No such file or directory: '{path}'")),
        ...
        OsFunction::Iterdir => {
            if let Some(entries) = get_virtual_dir_entries(&path) {
                ...
                let list: Vec<Object> = entries.into_iter().map(Object::Path).collect();
        ...
        OsFunction::Getenv => {
            ...
            if let Some(v) = value {
                Object::String(v.to_owned()).into()
            } else if matches!(default, Object::None) {
                Object::None.into()
            } else {
        ...
        OsFunction::Mkdir => {
            ...
            if is_virtual_dir(&path) {
                if exist_ok {
                    return Object::None.into();
                }
                return Exception::new(ExcType::OSError, Some(format!("[Errno 17] File exists: '{path}'"))).into();
            }
...
///
/// This function executes Python code via the Runner and validates the result
...
fn try_run_test(path: &Path, code: &str, expectation: &Expectation) -> Result<(), TestFailure> {
    ...
    if let Expectation::RefCounts(expected) = expectation {
        match Runner::new(code.to_owned(), &test_name, vec![], vec![]) {
            Ok(ex) => {
...
/// Try to run a test using Runner with external function support.
///
...
fn try_run_iter_test(path: &Path, code: &str, expectation: &Expectation) -> Result<(), TestFailure> {
    ...
    let exec = match Runner::new(code.to_owned(), &test_name, vec![], ext_functions) {
        Ok(e) => e,
        Err(parse_err) => {
            if let Expectation::Raise(expected) = expectation {
                let output = parse_err.py_repr();
...
fn run_iter_loop(exec: Runner) -> Result<Object, Exception> {
    let limits = ResourceLimits::new().max_recursion_depth(Some(TEST_RECURSION_LIMIT));
    let mut progress = exec.start(vec![], LimitedTracker::new(limits), &mut StdPrint)?;

    // Track pending async calls: (call_id, external_result)
    let mut pending_results: Vec<(u32, ExternalResult)> = Vec::new();
        ...
        match progress {
            ...
            RunProgress::FunctionCall {
                ...
                match dispatch_result {
                    ...
                    DispatchResult::AsyncReturn(result_value) => {
                        // Store the successful result for later resolution.
                        pending_results.push((call_id, ExternalResult::Return(result_value)));
                        // Continue execution with a pending future.
                        progress = state.run(OurosFuture, &mut StdPrint)?;
                    }
                    DispatchResult::AsyncError(error_exc) => {
                        // Store the failure for later resolution.
                        pending_results.push((call_id, ExternalResult::Error(error_exc)));
                        // Continue execution with a pending future
                        progress = state.run(OurosFuture, &mut StdPrint)?;
                    }
                }
            ...
            RunProgress::ResolveFutures(state) => {
                // If any pending future resolves with an error, feed one error at a time.
                // The VM routes one failed task through resume_with_exception per resume call.
                let first_error_idx = state.pending_call_ids().iter().find_map(|pending_id| {
                    pending_results
                        .iter()
                        .position(|(id, result)| id == pending_id && matches!(result, ExternalResult::Error(_)))
                });

                let results: Vec<(u32, ExternalResult)> = if let Some(idx) = first_error_idx {
                    vec![pending_results.remove(idx)]
                } else {
                    // No errors pending - resolve all available successful futures.
                    state
                        .pending_call_ids()
                        .iter()
                        .filter_map(|pending_id| {
                            pending_results
                                .iter()
                                .position(|(id, _)| id == pending_id)
...
///
/// Ouros supports top-level `await`, but CPython does not. This function transforms code
/// like:
///
/// ```python
/// async def foo():
...
fn import_run_traceback(py: Python<'_>) -> Bound<'_, PyModule> {
    // Add scripts directory to sys.path (tests run from crates/ouros/)
...
/// compares the result with the expected value. This ensures Ouros behaves
...
///
/// RefCounts tests are skipped as they're Ouros-specific.
...
/// Timeout duration for Ouros tests.
///
...
/// Test function that runs each fixture through Ouros.
///
/// Handles xfail with strict semantics: if a test is marked `xfail=ouros`, it must fail.
/// If an xfail test passes unexpectedly, that's an error.
fn run_test_cases_ouros(path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    if config.xfail_ouros {
        // Strict xfail: test must fail; if it passed, xfail should be removed
        assert!(
            result.is_err(),
            "[{test_name}] Test marked xfail=ouros passed unexpectedly. Remove xfail if the test is now fixed."
        );
