...
if sys.platform == 'ouros':
    assert csv.list_dialects().count('excel_tab') == 1, 'excel_tab should be registered'
    assert csv.list_dialects().count('unix_dialect') == 1, 'unix_dialect should be registered'
else:
