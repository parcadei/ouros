...
impl<'a> Compiler<'a> {
    ...
    /// Compiles module-level code for REPL execution with existing function IDs.
    ///
    /// Unlike [`Self::compile_module`], this starts the compiler with
    /// `existing_functions` so newly defined functions receive IDs that continue
    /// from previous REPL lines.
    pub fn compile_module_repl(
        nodes: &[PreparedNode],
        interns: &Interns,
        num_locals: u16,
        existing_functions: Vec<Function>,
    ) -> Result<CompileResult, CompileError> {
        let mut compiler = Compiler::new(interns, existing_functions);
        compiler.compile_block(nodes)?;
    ...
    fn compile_stmt(&mut self, node: &PreparedNode) -> Result<(), CompileError> {
        ...
        match node {
            ...
            Node::Import {
                ...
                binding,
                has_alias,
            } => self.compile_import(*module_name, binding, *has_alias),
            Node::ImportFrom {
                module_name,
                names,
                position,
    ...
    fn compile_function_def(&mut self, func_def: &PreparedFunctionDef) -> Result<(), CompileError> {
        ...
        let cell_base =
            u16::try_from(func_def.signature.total_slots()).expect("function parameter slot count exceeds u16");
        ...
        // Populate function metadata attributes.
        let doc_value = Self::function_docstring(&func_def.body).map_or(Value::None, Value::InternString);
        self.emit_set_function_attr_const(StaticStrings::DunderDoc, doc_value);
        self.emit_set_function_annotations_attr(&func_def.annotations)?;
    ...
    fn compile_class_def(&mut self, class_def: &ClassDef<PreparedFunctionDef>) -> Result<(), CompileError> {
        ...
        let class_body_func = Function::new_class_body(
            ...
            class_def.class_free_var_enclosing_slots.clone(),
            class_def.class_free_var_target_slots.clone(),
            body_code,
        );
    ...
    fn compile_lambda(&mut self, func_def: &PreparedFunctionDef) -> Result<(), CompileError> {
        ...
        // Lambda metadata mirrors CPython function objects.
        self.emit_set_function_attr_const(StaticStrings::DunderDoc, Value::None);
        self.emit_set_function_annotations_attr(&func_def.annotations)?;
    ...
    /// Compiles a prepared generator expression.
    ///
    /// Generator expressions are lowered during prepare into an anonymous generator
    /// function plus the eagerly evaluated outer iterator. At runtime we:
    ///
    /// 1. Evaluate the outer iterable and call `GetIter` in the enclosing scope
    /// 2. Create the anonymous function (with closure cells if needed)
    /// 3. Call it with the iterator argument to produce the lazy generator object
    fn compile_generator_expr(
        ...
        // Evaluate first iterable eagerly in the enclosing scope.
        self.compile_expr(outer_iter)?;
        self.code.emit(Opcode::GetIter);

        // Create the anonymous generator function object.
        self.compile_lambda(func_def)?;

        // Stack is [..., iter_arg, func]; rotate so callable is below the argument.
        self.code.emit(Opcode::Rot2);

        // Call with the implicit iterator parameter.
        self.code.set_location(expr_pos, None);
        self.code.emit_u8(Opcode::CallFunction, 1);
        Ok(())
    }

    /// Resolves a builtin module identifier during import compilation.
    ///
    /// Most modules are discovered through pre-interned `StaticStrings`. A small
    /// subset is looked up by literal module text so we can import them without
    /// extending `StaticStrings`.
    fn resolve_builtin_module(&self, module_name: StringId) -> Option<BuiltinModule> {
        BuiltinModule::from_string_id(module_name).or_else(|| match self.interns.get_str(module_name) {
            "binascii" => Some(BuiltinModule::Binascii),
            "cmath" => Some(BuiltinModule::Cmath),
            "codecs" => Some(BuiltinModule::Codecs),
            "collections.abc" => Some(BuiltinModule::CollectionsAbc),
            "html" => Some(BuiltinModule::Html),
            "argparse" => Some(BuiltinModule::Argparse),
            "logging" => Some(BuiltinModule::Logging),
            "numbers" => Some(BuiltinModule::Numbers),
            "shlex" => Some(BuiltinModule::Shlex),
            "fnmatch" => Some(BuiltinModule::Fnmatch),
            "tomllib" => Some(BuiltinModule::Tomllib),
            "types" => Some(BuiltinModule::Types),
            "typing_extensions" => Some(BuiltinModule::TypingExtensions),
            "urllib" => Some(BuiltinModule::Urllib),
            "urllib.parse" => Some(BuiltinModule::UrllibParse),
            "warnings" => Some(BuiltinModule::Warnings),
            "difflib" => Some(BuiltinModule::Difflib),
            "ipaddress" => Some(BuiltinModule::Ipaddress),
            "zlib" => Some(BuiltinModule::Zlib),
            "threading" => Some(BuiltinModule::Threading),
            "concurrent" => Some(BuiltinModule::Concurrent),
            "concurrent.futures" => Some(BuiltinModule::ConcurrentFutures),
            "token" => Some(BuiltinModule::Token),
            "tokenize" => Some(BuiltinModule::Tokenize),
            _ => None,
        })
    }
    ...
    fn compile_import(&mut self, module_name: StringId, binding: &Identifier, has_alias: bool) {
        let position = binding.position;
        self.code.set_location(position, None);

        // Look up the module by name
        if let Some(builtin_module) = self.resolve_builtin_module(module_name) {
            // CPython binds `import os.path` to `os` (the top-level package), while
            // `import os.path as p` binds the imported submodule to `p`.
            // We mirror this behavior for os.path using parse-time alias metadata.
            let builtin_module = if !has_alias && module_name == StaticStrings::OsPathMod {
                BuiltinModule::Os
            } else if !has_alias && self.interns.get_str(module_name) == "urllib.parse" {
                BuiltinModule::Urllib
            } else if !has_alias && self.interns.get_str(module_name) == "collections.abc" {
                BuiltinModule::Collections
            } else if !has_alias && self.interns.get_str(module_name) == "concurrent.futures" {
                BuiltinModule::Concurrent
            } else {
                builtin_module
            };
    ...
    /// Returns a function's docstring literal if its first statement is a string literal.
    fn function_docstring(body: &[PreparedNode]) -> Option<StringId> {
        let Some(Node::Expr(expr_loc)) = body.first() else {
            return None;
        };
        match expr_loc.expr {
            Expr::Literal(Literal::Str(string_id)) => Some(string_id),
            _ => None,
        }
    }

    /// Emits bytecode to assign a constant value to a function attribute.
    ///
    /// Requires a function object on top of the stack. Leaves that function object
    /// on the stack after the assignment.
    fn emit_set_function_attr_const(&mut self, attr: StaticStrings, value: Value) {
        let attr_idx = u16::try_from(StringId::from(attr).index()).expect("attr id exceeds u16");
        self.code.emit(Opcode::Dup);
        let value_const = self.code.add_const(value);
        self.code.emit_u16(Opcode::LoadConst, value_const);
        self.code.emit(Opcode::Rot2);
        self.code.emit_u16(Opcode::StoreAttr, attr_idx);
    }

    /// Emits bytecode to assign `__annotations__` to a function.
    ///
    /// Requires a function object on top of the stack. Leaves that function object
    /// on the stack after the assignment.
    fn emit_set_function_annotations_attr(&mut self, annotations: &[(StringId, ExprLoc)]) -> Result<(), CompileError> {
        let attr_idx = u16::try_from(StringId::from(StaticStrings::DunderAnnotations).index())
            .expect("annotation attr id exceeds u16");
        self.code.emit(Opcode::Dup);
        for (name_id, expr) in annotations {
            let key_const = self.code.add_const(Value::InternString(*name_id));
            self.code.emit_u16(Opcode::LoadConst, key_const);
            self.compile_expr(expr)?;
        }
        self.code.emit_u16(
            Opcode::BuildDict,
            u16::try_from(annotations.len()).expect("annotation count exceeds u16"),
        );
        self.code.emit(Opcode::Rot2);
        self.code.emit_u16(Opcode::StoreAttr, attr_idx);
        Ok(())
    ...
    fn compile_expr(&mut self, expr_loc: &ExprLoc) -> Result<(), CompileError> {
        ...
        match &expr_loc.expr {
            ...
            Expr::DictUnpack(items) => {
                // Preserve Python's left-to-right dict literal semantics:
                // start from {}, then apply pair inserts and mapping unpacks in order.
                ...
                for item in items {
                    match item {
                        ...
                        DictLiteralItem::Unpack { mapping } => {
                            self.compile_expr(mapping)?;
                            self.code.emit(Opcode::DictUpdate);
                        }
                    }
                }
            }
            ...
            Expr::GeneratorExp { outer_iter, func_def } => {
                self.compile_generator_expr(outer_iter, func_def, expr_loc.position)?;
            }

            Expr::GeneratorExpRaw { .. } => {
                // GeneratorExpRaw should be converted to GeneratorExp during prepare phase
            ...
            Expr::Yield { value } => {
                // Compile the yielded value (or LoadNone for bare yield)
            ...
            Expr::YieldFrom { value } => {
                // Evaluate delegated iterable, convert to iterator, then delegate.
                self.compile_expr(value)?;
                self.code.emit(Opcode::GetIter);
                // Use the full expression location for traceback caret range.
                self.code.set_location(expr_loc.position, None);
                self.code.emit(Opcode::YieldFrom);
            }
        }
        Ok(())
    }
    ...
    /// - `break` doesn't need to pop the iterator (nothing extra on stack)
    fn compile_while(
        &mut self,
        test: &ExprLoc,
        body: &[PreparedNode],
    ...
    /// Bytecode structure:
    /// ```text
    /// BUILD_LIST_HINT k     ; or BUILD_LIST 0 when size is unknown
    /// <compile first iter>
    /// GET_ITER
    /// loop_start:
    ///   FOR_ITER end_loop
    ...
    fn compile_list_comp(&mut self, elt: &ExprLoc, generators: &[Comprehension]) -> Result<(), CompileError> {
        // Build empty list (with capacity hint when we can infer exact cardinality).
        if let Some(capacity_hint) = Self::list_comp_capacity_hint(generators)
            && capacity_hint > 0
            && let Ok(capacity_hint_i64) = i64::try_from(capacity_hint)
        {
            let const_idx = self.code.add_const(Value::Int(capacity_hint_i64));
            self.code.emit_u16(Opcode::BuildListHint, const_idx);
        } else {
    ...
    /// Returns an exact list-length hint for comprehensions we can size statically.
    ///
    /// We currently infer only the no-filter case where every generator iterates
    /// over a literal `range(...)` call. For those comprehensions, the output
    /// cardinality is exactly the cartesian product of generator lengths.
    fn list_comp_capacity_hint(generators: &[Comprehension]) -> Option<usize> {
        let mut total = 1usize;
        for generator in generators {
            if !generator.ifs.is_empty() {
                return None;
            }
            let iter_len = Self::literal_range_len_hint(&generator.iter)?;
            total = total.checked_mul(iter_len)?;
        }
        Some(total)
    }

    /// Returns the exact length for literal `range(...)` iter expressions.
    ///
    /// Supported forms:
    /// - `range(stop)`
    /// - `range(start, stop)`
    ...
    fn literal_range_len_hint(iter: &ExprLoc) -> Option<usize> {
        let Expr::Call {
            callable: Callable::Builtin(Builtins::Type(crate::types::Type::Range)),
            args,
        } = &iter.expr
        else {
            return None;
        ...
        match args.as_ref() {
            ArgExprs::One(stop) => Self::range_len_from_literal_args(0, Self::literal_i64(stop)?, 1),
            ArgExprs::Two(start, stop) => {
                Self::range_len_from_literal_args(Self::literal_i64(start)?, Self::literal_i64(stop)?, 1)
            }
            ArgExprs::Args(args) if args.len() == 3 => Self::range_len_from_literal_args(
                Self::literal_i64(&args[0])?,
                Self::literal_i64(&args[1])?,
                Self::literal_i64(&args[2])?,
            ),
            _ => None,
        }
    }
    ...
    ///
    /// All arguments must be integer literals.
    fn literal_i64(expr: &ExprLoc) -> Option<i64> {
        match expr.expr {
    ...
    /// Computes the exact cardinality of `range(start, stop, step)`.
    fn range_len_from_literal_args(start: i64, stop: i64, step: i64) -> Option<usize> {
        if step == 0 {
            return None;
        }
    ...
    /// 3. Store current value to target (single variable or tuple unpacking)
    ...
    /// Compiles storage of an unpack target - identifier, subscript, nested tuple, or starred.
    ///
    /// For single identifiers: emits a simple store.
    /// For subscript targets: leaves the unpacked value on stack and stores via `StoreSubscr`.
        ...
        for part in parts {
            match part {
                ...
                FStringPart::Interpolation {
                    ...
                    // For interpolation without a format spec, use builtin str()/repr()
                    // opcodes so user-defined __str__/__repr__ dispatch follows the same
                    // VM call path as regular builtin calls.
                    if format_spec.is_none() {
                        match effective_conversion {
                            ConversionFlag::None | ConversionFlag::Str => {
                                let str_type_id =
                                    Type::Str.callable_to_u8().expect("str should have a callable type id");
                                self.code.emit_call_builtin_type(str_type_id, 1);
                            }
                            ConversionFlag::Repr => {
                                self.code.emit_call_builtin_function(BuiltinsFunctions::Repr as u8, 1);
                            }
                            ConversionFlag::Ascii => {
                                // Keep !a on the existing FormatValue path.
                                let flags = self.compile_format_value(
                        ...
                        // `f'{value:spec}'` should dispatch through `format(value, spec)` so
                        // user-defined `__format__` methods run through the regular VM call path.
                        // Keep `FORMAT_VALUE` for conversions and debug forms where conversion
                        // ordering has dedicated opcode semantics.
                        if matches!(effective_conversion, ConversionFlag::None) && debug_prefix.is_none() {
                            self.compile_format_spec_as_string(
                                format_spec
                                    .as_ref()
                                    .expect("checked Some(format_spec) in surrounding branch"),
                            )?;
                            self.code.emit_call_builtin_function(BuiltinsFunctions::Format as u8, 2);
                        } else {
                            // Emit FormatValue with appropriate flags.
                            // Pass is_debug=true when debug_prefix is present so VM knows to
                            // apply format spec to original value type (not string conversion).
                            let is_debug = debug_prefix.is_some();
                            let flags =
                                self.compile_format_value(effective_conversion, format_spec.as_ref(), is_debug)?;
                            self.code.emit_u8(Opcode::FormatValue, flags);
                        }
                    }
                    count += 1;
    ...
    /// - bit 3: is debug format (with '=' prefix)
    fn compile_format_value(
        ...
        // Debug flag: bit 3
        let debug_bit = if is_debug { 0x08 } else { 0 };

        match format_spec {
            None => Ok(conv_bits | debug_bit),
    ...
    /// Compiles a format-spec value as a plain `str` on the stack.
    ///
    /// This is used when lowering to `format(value, spec)` so user-defined
    /// `__format__` receives the exact source spec text for static specs.
    fn compile_format_spec_as_string(&mut self, format_spec: &FormatSpec) -> Result<(), CompileError> {
        match format_spec {
            ...
            FormatSpec::Dynamic(dynamic_parts) => {
                let part_count = self.compile_fstring_parts(dynamic_parts)?;
                if part_count > 1 {
                    self.code.emit_u16(Opcode::BuildFString, part_count);
                }
    ...
    fn compile_try(&mut self, try_block: &Try<PreparedNode>) -> Result<(), CompileError> {
        ...
        let finally_with_return_start = if has_finally {
            ...
            if !finally_target.continue_jumps.is_empty() {
                ...
                // (the iterator was on stack at try entry, continue didn't remove it)
...
impl CompileError {
    ...
    pub fn into_python_exc(self, filename: &str, source: &str) -> Exception {
        ...
        Exception::new_full(self.exc_type, Some(self.message.into_owned()), vec![frame])
    }
}
