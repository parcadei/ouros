...
/// Public representation of a Ouros exception.
...
/// Display implementation for Exception should exactly match python traceback format.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(msg) = &self.message {
            if self.exc_type == ExcType::KeyError {
                write!(f, "{}: {}", self.exc_type, StringRepr(msg))
            } else {
...
impl Exception {
    /// Create a new Exception with the given exception type and message.
    ///
    ...
    /// Optional exception message explaining what went wrong.
    ///
    /// This takes ownership of the Exception and returns an owned String.
    ///
    /// Equivalent of python's `exc.args[0]`
    #[must_use]
...
/// # Caret Markers
///
/// Ouros uses only `~` characters for caret markers in tracebacks, unlike CPython 3.11+
/// which uses `~` for the function name and `^` for arguments (e.g., `~~~~~~~~~~~^^^^^^^^^^^`).
/// This simplification is intentional - Ouros marks the entire expression span uniformly.
#[derive(Debug, Clone, PartialEq)]
