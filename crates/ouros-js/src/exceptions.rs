//! Exception types for the Ouros TypeScript/JavaScript bindings.
//!
//! This module provides thin napi wrappers around Ouros's internal exceptions.
//! The JavaScript wrapper layer (`wrapper.js`) is responsible for converting
//! these into proper JS `Error` subclasses (`OurosSyntaxError`, `OurosRuntimeError`).
//!
...
//! ## Architecture
//!
//! - `JsOurosException` (JS name: `SandboxException`): Thin wrapper around `ouros::Exception`.
//!   The JS wrapper checks `exception.typeName` to distinguish syntax errors from runtime errors.
//! - `OurosTypingError` (JS name: `SandboxTypingError`): Wraps `TypeCheckingDiagnostics` for
//!   static type checking errors. This is separate because type errors come from static analysis,
...
use ouros::StackFrame;
use ouros_type_checking::TypeCheckingDiagnostics;
use serde::{Deserialize, Serialize};

// =============================================================================
// JsOurosException - Thin wrapper around core Exception
// =============================================================================

/// Wrapper around core `Exception` for napi bindings (JS name: `SandboxException`).
///
/// This is a thin newtype wrapper that exposes the necessary getters for the
/// JavaScript wrapper to construct appropriate error types (`OurosSyntaxError`
/// or `OurosRuntimeError`) based on the exception type.
#[napi(js_name = "SandboxException")]
pub struct JsOurosException(ouros::Exception);

impl fmt::Display for JsOurosException {
...
impl JsOurosException {
    /// Creates a new JsOurosException from a core Exception.
    #[must_use]
    pub fn new(exc: ouros::Exception) -> Self {
        Self(exc)
    }
}

// =============================================================================
// OurosTypingError - Raised when type checking finds errors
// =============================================================================
...
#[napi(js_name = "SandboxTypingError")]
...
impl fmt::Display for OurosTypingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl OurosTypingError {
    /// Creates a OurosTypingError from a TypeCheckingDiagnostics.
    #[must_use]
...
impl Frame {
    /// Creates a `Frame` from Ouros's `StackFrame`.
    #[must_use]
