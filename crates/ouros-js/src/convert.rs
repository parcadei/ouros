//! Type conversion between Ouros's `OurosObject` and JavaScript values via napi-rs.
//!
//! This module provides bidirectional conversion using native napi-rs APIs:
//! - `ouros_to_js`: Convert Ouros's `OurosObject` to a JavaScript value
//! - `js_to_ouros`: Convert a JavaScript value to Ouros's `OurosObject`
//!
//! ## Type Mappings
//!
//! ### Native JS types (bidirectional):
//! - `OurosObject::None` ↔ `null`
//! - `OurosObject::Bool` ↔ `boolean`
//! - `OurosObject::Int` ↔ `number` (if within safe integer range) or `BigInt`
//! - `OurosObject::BigInt` ↔ `BigInt`
//! - `OurosObject::Float` ↔ `number` (including `NaN`, `Infinity`, `-Infinity`)
//! - `OurosObject::String` ↔ `string`
//! - `OurosObject::Bytes` ↔ `Buffer` (Node.js)
//! - `OurosObject::List` ↔ `Array`
//! - `OurosObject::Dict` ↔ `Map` (preserves key types and insertion order)
//! - `OurosObject::Set` ↔ `Set`
//! - `OurosObject::FrozenSet` ↔ `Set` (JS has no frozen set)
//!
//! ### Marked JS types (with `__ouros_type__` property):
//! - `OurosObject::Ellipsis` → `{ __ouros_type__: 'Ellipsis' }`
//! - `OurosObject::Tuple` → `Array` with `__tuple__: true`
//! - `OurosObject::Exception` → `{ __ouros_type__: 'Exception', excType, message }`
//! - `OurosObject::Type` → `{ __ouros_type__: 'Type', value }`
//! - `OurosObject::BuiltinFunction` → `{ __ouros_type__: 'BuiltinFunction', value }`
//! - `OurosObject::Dataclass` → `{ __ouros_type__: 'Dataclass', name, fields, ... }`
//! - `OurosObject::Repr` → plain `string`
//! - `OurosObject::Cycle` → placeholder `string`

use std::collections::HashMap;

use napi::bindgen_prelude::*;
use num_bigint::BigInt as NumBigInt;
use ouros::{DictPairs, ExcType, Object as OurosObject};
...
///
/// This allows `ouros_to_js` to return dynamically typed JS values.
pub struct JsOurosObject<'env>(pub(crate) Unknown<'env>);

impl JsOurosObject<'_> {
...
impl ToNapiValue for JsOurosObject<'_> {
    unsafe fn to_napi_value(env: sys::napi_env, val: Self) -> Result<sys::napi_value> {
        Unknown::to_napi_value(env, val.0)
    }
}

/// Converts Ouros's `OurosObject` to a JavaScript value using native napi-rs APIs.
///
...
pub fn ouros_to_js<'e>(obj: &OurosObject, env: &'e Env) -> Result<JsOurosObject<'e>> {
    let unknown = match obj {
        OurosObject::None => create_js_null(env)?,
        OurosObject::Ellipsis => create_js_ellipsis(env)?,
        OurosObject::Bool(b) => create_js_bool(*b, env)?,
        OurosObject::Int(i) => create_js_int(*i, env)?,
        OurosObject::BigInt(bi) => create_js_bigint(bi, env)?,
        OurosObject::Float(f) => env.create_double(*f)?.into_unknown(env)?,
        OurosObject::String(s) => env.create_string(s)?.into_unknown(env)?,
        OurosObject::Bytes(bytes) => create_js_buffer(bytes, env)?,
        OurosObject::List(items) => create_js_array(items, env)?.into_unknown(env)?,
        OurosObject::Tuple(items) => create_js_tuple(items, env)?,
        // NamedTuple is converted to a tuple (loses named access in JS)
        OurosObject::NamedTuple { values, .. } => create_js_tuple(values, env)?,
        OurosObject::Dict(pairs) => create_js_map(pairs, env)?,
        OurosObject::Set(items) | OurosObject::FrozenSet(items) => create_js_set(items, env)?,
        OurosObject::Exception { exc_type, arg } => create_js_exception(*exc_type, arg.as_deref(), env)?,
        OurosObject::Type(t) => create_js_type_marker(&t.to_string(), env)?,
        OurosObject::BuiltinFunction(f) => create_js_builtin_function_marker(&f.to_string(), env)?,
        OurosObject::Dataclass {
            name,
            type_id,
            field_names,
            attrs,
        ...
        OurosObject::Proxy(proxy_id) => create_js_proxy_marker(*proxy_id, env)?,
        OurosObject::Path(p) => env.create_string(p)?.into_unknown(env)?,
        OurosObject::Repr(s) | OurosObject::Cycle(_, s) => env.create_string(s)?.into_unknown(env)?,
    };
    Ok(JsOurosObject(unknown))
}
...
/// Creates a native JS Array from Ouros list items, recursively converting each element.
fn create_js_array<'e>(items: &[OurosObject], env: &'e Env) -> Result<Array<'e>> {
    let mut arr = env.create_array(items.len().try_into().expect("array size overflows u32"))?;
    for (i, item) in items.iter().enumerate() {
        let js_item = ouros_to_js(item, env)?;
...
fn create_js_tuple<'e>(items: &[OurosObject], env: &'e Env) -> Result<Unknown<'e>> {
    let mut arr = create_js_array(items, env)?;
    arr.set_named_property("__tuple__", true)?;
    arr.into_unknown(env)
}

/// Creates a native JS `Map` from Ouros dict pairs, recursively converting keys and values.
///
...
/// Creates a native JS Set from Ouros set items.
fn create_js_set<'e>(items: &[OurosObject], env: &'e Env) -> Result<Unknown<'e>> {
    let global = env.get_global()?;
...
/// Creates a JS object representing Ellipsis: `{ __ouros_type__: 'Ellipsis' }`.
fn create_js_ellipsis(env: &Env) -> Result<Unknown<'_>> {
    let mut obj = Object::new(env)?;
    obj.set_named_property("__ouros_type__", "Ellipsis")?;
...
fn create_js_builtin_function_marker<'e>(func_str: &str, env: &'e Env) -> Result<Unknown<'e>> {
    let mut obj = Object::new(env)?;
    obj.set_named_property("__ouros_type__", "BuiltinFunction")?;
    obj.set_named_property("value", func_str)?;
    obj.into_unknown(env)
}

/// Creates a JS object representing a proxy handle.
fn create_js_proxy_marker(proxy_id: u32, env: &Env) -> Result<Unknown<'_>> {
    let mut obj = Object::new(env)?;
    obj.set_named_property("__ouros_type__", "Proxy")?;
    obj.set_named_property("proxyId", proxy_id)?;
    obj.into_unknown(env)
}
...
fn create_js_dataclass<'e>(
    ...
    let mut obj = Object::new(env)?;
    obj.set_named_property("__ouros_type__", "Dataclass")?;
    ...
    let attrs_map: HashMap<&str, &OurosObject> = attrs
        .into_iter()
        .filter_map(|(k, v)| {
            if let OurosObject::String(key) = k {
                Some((key.as_str(), v))
            } else {
                None
            }
    ...
    for field_name in field_names {
        if let Some(value) = attrs_map.get(field_name.as_str()) {
            let js_value = ouros_to_js(value, env)?;
...
// =============================================================================
// JS to Ouros conversion
// =============================================================================

/// Converts a JavaScript value to Ouros's `OurosObject`.
///
...
/// - `Map` → `Dict`
/// - `Set` → `Set`
/// - `OurosObject` with `__ouros_type__` → corresponding Ouros type
/// - `OurosObject` → `Dict` (string keys only)
pub fn js_to_ouros(value: Unknown<'_>, env: Env) -> Result<OurosObject> {
    let value_type = value.get_type()?;

    match value_type {
        ValueType::Null | ValueType::Undefined => Ok(OurosObject::None),
        ValueType::Boolean => {
            let b: bool = value.coerce_to_bool()?;
            Ok(OurosObject::Bool(b))
        }
        ValueType::Number => {
            ...
            if n.fract() == 0.0 && n >= i64::MIN as f64 && n <= i64::MAX as f64 {
                ...
                return Ok(OurosObject::Int(n as i64));
            }
            Ok(OurosObject::Float(n))
        }
        ValueType::BigInt => {
            ...
            // Try to fit in i64
            if let Ok(i) = i64::try_from(&bi) {
                Ok(OurosObject::Int(i))
            } else {
                Ok(OurosObject::BigInt(bi))
            }
        }
        ValueType::String => {
            let s: String = value.coerce_to_string()?.into_utf8()?.into_owned()?;
            Ok(OurosObject::String(s))
        }
        ValueType::Object => {
            ...
            if obj.is_buffer()? {
                ...
                return Ok(OurosObject::Bytes(buffer.to_vec()));
            }

            // Check if it's a Map
            if is_js_map(&obj, env)? {
                return js_map_to_ouros(obj, env);
            }

            // Check if it's a Set
            if is_js_set(&obj, env)? {
                return js_set_to_ouros(obj, env);
            }

            // Check if it's an Array
            if obj.is_array()? {
                return js_array_to_ouros(obj, env);
            }

            // Check for __ouros_type__ marker
            if let Some(ouros_type) = get_string_property(&obj, "__ouros_type__")? {
                return js_marked_object_to_ouros(&obj, &ouros_type, env);
            }

            // Plain object → Dict (with string keys)
            js_object_to_ouros_dict(obj, env)
        }
        ValueType::Function | ValueType::Symbol | ValueType::External => {
            // These JS types don't have Ouros equivalents
            Err(Error::from_reason(format!(
                "Cannot convert JS {value_type:?} to Ouros value"
            )))
        }
...
/// Converts a JS Map to `OurosObject::Dict`.
fn js_map_to_ouros(map: Object, env: Env) -> Result<OurosObject> {
    ...
    loop {
        ...
        let ouros_key = js_to_ouros(key, env)?;
        let ouros_value = js_to_ouros(value, env)?;
        pairs.push((ouros_key, ouros_value));
    }

    Ok(OurosObject::dict(pairs))
}

/// Converts a JS Set to `OurosObject::Set`.
fn js_set_to_ouros(set: Object, env: Env) -> Result<OurosObject> {
    ...
    loop {
        ...
        items.push(js_to_ouros(value, env)?);
    }

    Ok(OurosObject::Set(items))
}

/// Converts a JS Array to `OurosObject::List` or `OurosObject::Tuple`.
fn js_array_to_ouros(arr: Object, env: Env) -> Result<OurosObject> {
    ...
    for i in 0..length {
        ...
        items.push(js_to_ouros(element, env)?);
    }

    if is_tuple {
        Ok(OurosObject::Tuple(items))
    } else {
        Ok(OurosObject::List(items))
    }
}

/// Converts a JS object with `__ouros_type__` marker to the appropriate `OurosObject`.
fn js_marked_object_to_ouros(obj: &Object, ouros_type: &str, env: Env) -> Result<OurosObject> {
    match ouros_type {
        "Ellipsis" => Ok(OurosObject::Ellipsis),
        "Exception" => {
        ...
        "Dataclass" => {
            ...
            for field_name in &field_names {
                if let Some(value) = fields_obj.get_named_property::<Option<Unknown>>(field_name.as_str())? {
                    let ouros_value = js_to_ouros(value, env)?;
                    attrs_vec.push((OurosObject::String(field_name.clone()), ouros_value));
                }
            }
        ...
        "Proxy" => {
            let proxy_id: u32 = obj.get_named_property("proxyId")?;
            Ok(OurosObject::Proxy(proxy_id))
        }
        _ => {
...
/// Converts a plain JS object to `OurosObject::Dict`.
///
...
fn js_object_to_ouros_dict(obj: Object, env: Env) -> Result<OurosObject> {
    ...
    for i in 0..length {
        ...
        let ouros_value = js_to_ouros(value, env)?;
        pairs.push((OurosObject::String(key_str), ouros_value));
    }
