...
so that iter mode tests produce identical results in both Ouro and CPython.

This module is shared between:
- scripts/run_traceback.py (for traceback tests)
- crates/ouro/tests/datatest_runner.rs (via include_str! for CPython execution)
"""
...
async def async_raise(exc_type: str, message: str) -> object:
    """Async function that raises the requested exception.

    Mirrors `dispatch_external_call(..., "async_raise", ...)` in the Rust test
    runner so `# call-external` tests behave identically on Ouro and CPython.
    """
    exc_types: dict[str, type[Exception]] = {
        'ValueError': ValueError,
        'TypeError': TypeError,
...
if not hasattr(os, '_ouro_original_getenv'):
    os._ouro_original_getenv = os.getenv  # pyright: ignore[reportAttributeAccessIssue]
    os._ouro_original_environ = os.environ  # pyright: ignore[reportAttributeAccessIssue]

_original_getenv = os._ouro_original_getenv  # pyright: ignore[reportAttributeAccessIssue,reportUnknownVariableType,reportUnknownMemberType]
_original_environ = os._ouro_original_environ  # pyright: ignore[reportAttributeAccessIssue,reportUnknownVariableType,reportUnknownMemberType]
...
def _virtual_getenv(key: str, default: str | None = None) -> str | None:
    ...
    or None if not in the virtual env (ignoring default for these keys to match Ouro behavior).
...
class VirtualEnviron:
    ...
    in both Ouro (virtual env) and CPython (real env + virtual overlay).
    """
